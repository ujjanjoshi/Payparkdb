...
class IncomingDetails:
    def __init__(self, root):
        ...
        # Frame_in.place(x=0,y=0,width=700,height=500)
        ...
        # Frame_table.pack()
        # Frame_table.place(x=0, y=70, width=700, height=430)
        ...
        my_frame1['columns'] = ('ParkingNo', 'NumberPlate','Incoming')

        # format our column
        # my_frame.column("#0", width=0)
        my_frame1.column("ParkingNo", anchor=CENTER, width=100)
        my_frame1.column("NumberPlate", anchor=CENTER, width=220)
        my_frame1.column("Incoming", anchor=CENTER, width=220)
        style = ttk.Style()
        ...
        for rg in range(len(data)):
            for gr in range(len(data)):
                print(data.loc[rg][gr + 2], data.loc[rg][gr + 1], data.loc[rg][gr + 3])
                my_frame1.insert(parent='', index='end', iid=count, text='', values=(
                    data.loc[rg][gr+2], data.loc[rg][gr + 1], data.loc[rg][gr + 3]))
